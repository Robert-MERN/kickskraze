...
const View_order_drawer = ({ drawer_state, toggle_drawer, toggle_modal, axios }) => {



    const { get_all_orders_api, set_orders, set_dispatched_orders, get_order_api, order_id, set_order_id, update_order_api, set_API_loading, set_snackbar_alert } = useStateContext();
    ...
    useEffect(() => {
        if (drawer_state.view_order_drawer && order_id) {
            get_order_api(axios, order_id, set_confirmed_order, set_is_loading);
        } else {
            set_confirmed_order(null);
            set_errors(default_errors);
        }
    }, [order_id, drawer_state.view_order_drawer]);
                                ...
                                : confirmed_order ?
                                                    ...
                                                    {confirmed_order.purchase.map((item) => (
                                                        <div key={item._id} className="w-full  my-1" >
                                                            ...
                                                            <div className='py-[12px] flex w-full justify-start'>
                                                                <div
                                                                    className={`select-none border border-gray-200 rounded-md flex items-center gap-4 md:gap-6`}
                                                                >
                                                                    <button
                                                                        onClick={() => updateOrderQty(item._id, -1)}
                                                                        className='px-[7px] md:px-[11px] py-[7px] md:py-[9px] text-stone-800 active:text-stone-400 transition-all'
                                                                    >
                                    ...
                                    <div className="w-full h-[70vh] flex flex-col justify-center items-center">
                                        ...
                                        <div className='my-[30px] w-full flex justify-center items-center'>
                                            <button onClick={close_drawer} className='w-full py-[12px] text-white bg-black font-semibold text-[14px] md:text-[15px]  transition-all duration-300 hover:opacity-70 active:scale-[.97] px-[50px] flex justify-center gap-3 items-center'>
                                                <ShoppingCartCheckoutIcon className='text-[19px]' />  GO BACK
                                            </button>
                                        </div>
                                    </div>
...
export default View_order_drawer