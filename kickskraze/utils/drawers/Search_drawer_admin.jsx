...
const Search_drawer_admin = ({ drawer_state, toggle_drawer, get_all_products_api, axios }) => {
  ...
  useEffect(() => {
    ...
    if (drawer_state.search_drawer_admin && debouncedTerm !== null && store_name) {
      fetchResults();
    }
  }, [debouncedTerm, drawer_state.search_drawer_admin, store_name]);
  ...
  useEffect(() => {
    const fetch = async () => {
      if (drawer_state.search_drawer_admin) {
  ...
  }, [drawer_state.search_drawer_admin, store_name]);
  ...
  const view_more_btn = () => {
    toggle_drawer("search_drawer_admin")
    ...
    >
      <div className='w-[90vw] md:w-[50vw] text-stone-950 transition-all duration-300'>
        ...
        <div className='flex justify-between w-full items-center py-[12px] px-[20px]' >
          ...
          <button onClick={() => toggle_drawer("search_drawer_admin")} className='active:opacity-75' >
...
export default Search_drawer_admin